  ...
  const fetchData = async (e) => {
    ...
    try {
      const response = await fetch(
        `/api/instagram/getinstagramuserdata?username=${inputValue}`
      );
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
      const result = await response.json();
      setData(result.follower_counts); // Ensure data is in the expected format
      console.log(result.follower_counts);
    } catch (error) {
        ...
        <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
          ...
          {loading ? (
            ...
            <div className="h-[400px]">
              <ImpressionsBarChart data={data} name="Reach" />
            </div>
          )}
        </div>
      </div>
